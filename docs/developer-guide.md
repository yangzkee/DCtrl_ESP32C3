...
本项目是一套面向 ESP32-C3 循线小车的 ESP-IDF 固件骨架，其模块边界把目标芯片差异集中在板级配置中。
...
- `motion_contracts`
  ...
  - `Independent Test`：C3 构建，以及对档位映射、循线质量和偏移曲线的确定性检查。
...
- 为 ESP32-C3、传感器和底盘控制器提供稳定的供电预算。电机电源不得倒灌进 ESP 模块。
...
- C3 底盘接线：`GPIO4` -> 底盘 RX，`GPIO5` <- 底盘 TX，外加共地 GND。
- C3 循线传感器接线：`GPIO2` -> 循线传感器 RX，`GPIO3` <- 循线传感器 TX，外加共地 GND。

在 `/dev/cu.usbmodem11201` 上进行 C3 板冒烟测试，确认固件以 `esp32c3-portable-dual-uart-profile` 启动。C3 循线传感器接线后来从 `GPIO20/GPIO21` 移走，因为现场遥测显示循线 UART 读回的是 `$0,0,1#` 而非模块的 `$D,...#` 帧。未来的 C3 引脚改动请保持在 `components/board/board_profile.c` 内。
...
该安装包含 `esp32c3` 目标工具链。下面的项目辅助脚本会加载 `export.sh`、进入项目根目录，并把参数转发给 `idf.py`：
...
```sh
./scripts/idf.sh -B build-esp32c3 -DSDKCONFIG=sdkconfig.esp32c3 build
./scripts/idf.sh -B build-esp32c3 -DSDKCONFIG=sdkconfig.esp32c3 -p /dev/cu.usbmodem11201 flash monitor
```

当前 ESP32-C3 构建验证通过，并生成 `build-esp32c3/DCar-Liner.bin`。

固件使用自定义的 4 MB 单 app 分区表 `partitions_single_4mb.csv`。只有一个 `factory` app 槽（`0x3B0000`）；空中升级（OTA）已被移除，因此固件只能通过 USB 或 web-serial 烧录更新。
