...
- `LINE_LOST`: no active line bits while running; the policy commands zero linear speed plus an origin-relative recovery sweep. The lost-line pose is the zero point. It probes left/right at `6, 12, 24 deg`; when one side reaches `45 deg`, that same side becomes the recovery side and grows to `66` then `90 deg`. If the line still does not return, it keeps the same direction until the estimated deviation from the lost-line pose reaches `360 deg`; if the line still does not return, the plan requests `STOP_TO_IDLE`.
